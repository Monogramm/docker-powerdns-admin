...
if [ -n "${DOCKER_WEB_CONTAINER}" ]; then
    ...
    DOCKER_WEB_CONTAINER_HEALTH_URL=http://${DOCKER_WEB_CONTAINER}:${DOCKER_WEB_PORT:-80}/login
    log "Checking Health API: ${DOCKER_WEB_CONTAINER_HEALTH_URL}"
    curl "${DOCKER_WEB_CONTAINER_HEALTH_URL}"
    curl --fail "${DOCKER_WEB_CONTAINER_HEALTH_URL}" | grep -q -e 'Log In - PowerDNS-Admin' || exit 1
fi
